...
class Paper {
    ...
    private EdgeAnimation mAnimationTop = new EdgeAnimation();
    private EdgeAnimation mAnimationBottom = new EdgeAnimation();
    private int mHeight;
    ...
    public void overScroll(float distance) {
        ...
        if (distance < 0) {
            mAnimationTop.onPull(-distance);
        } else {
            mAnimationBottom.onPull(distance);
        }
    }
    ...
    public void edgeReached(float velocity) {
        ...
        if (velocity < 0) {
            mAnimationBottom.onAbsorb(-velocity);
        } else {
            mAnimationTop.onAbsorb(velocity);
        }
    }

    public void onRelease() {
        mAnimationTop.onRelease();
        mAnimationBottom.onRelease();
    }

    public boolean advanceAnimation() {
        // Note that we use "|" because we want both animations get updated.
        return mAnimationTop.update() | mAnimationBottom.update();
    }
    ...
    public float[] getTransform(Rect rect, float scrollY) {
        float top = mAnimationTop.getValue();
        float bottom = mAnimationBottom.getValue();
        float screenY = rect.centerY() - scrollY;
        // We linearly interpolate the value [top, bottom] for the screenY
        // range int [-1/4, 5/4]*mHeight. So if part of the thumbnail is outside
